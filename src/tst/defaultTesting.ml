...
let string_match r s =
  try Str.search_forward r s 0 |> fun _ -> true with Not_found -> false
    ...
    try
      ...
      | Some intset ->
          let s =
            IntSet.fold
              (fun j s1 ->
                s1 ^ "#" ^ string_of_assignment asgmt ^ string_of_int j )
              intset ""
          in
          fname ^ " " ^ s
      | None -> fname
...
let testCasesFromFiles options phase tester files =
  let nameFormatter =
    get_feature_hashtags_for_phase options.feature_map phase
  in
  let filtered_based_on_feature_sets =
    List.filter
      (feature_set_match options.fs_exclusive options.fs_regexp
         options.feature_map phase )
      files
  in
  List.map
    (testFileWithTester nameFormatter tester)
    filtered_based_on_feature_sets
...
let defaultTestsForPhases options phases =
  let only_pattern_matcher =
    ...
    | None -> fun _ -> true
    | Some r -> string_match r
  in
  let extraMatcher fname = only_pattern_matcher fname in
  ...
  let defaultPositives phase =
    testCasesFromFiles options phase
      (defaultTestPos phase options.overwrite)
      (posTests phase)
  in
  let defaultNegatives phase =
    testCasesFromFiles options phase
      ((defaultTestNeg phase) phase options.overwrite)
      (negTests phase)
  in
  ...
  let defaultTests phase =
    ...
    ; (name ^ "-", defaultNegatives phase) ]
  in
  phases |> List.map defaultTests |> List.concat
