...
let only_flag =
  let doc =
    "Only pick filenames matching the regular expression $(i,REGEXP)."
  in
...
let featuresetregexp_flag =
  let doc =
    "Choose feature sets matching the regular expression $(i,REGEXP)."
  in
  Arg.(
    value
...
let () =
  ...
  match Cmd.eval_peek_opts (init_testsets_t featureSetMap) with
  ...
  *)
  | Some tests, _ ->
      ( try
          Alcotest.run_with_args "TigerCompiler" dummy_testsets_t tests
            ~and_exit:false
      ...
      Printf.printf
        ...
        diff_file
  | _ ->
      Alcotest.run_with_args "Tiger compiler" dummy_testsets_t
        [] (* let the tool show the exception *)
