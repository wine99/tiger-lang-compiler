...
let checkJsonPath () =
  if not (Sys.file_exists json_file) then
    failwith @@ "cannot find json file " ^ json_file
...
let normalize_feature_string s' =
  ...
  let s'' =
    if last_char = '+' || last_char = '-' then String.sub s' 0 (n - 1)
    else s'
  in
...
let string_to_featureset s =
  ...
  let asgmt =
    ...
    | _ -> failwith @@ "Unrecognized featureset " ^ s
  in
...
let string_list_to_featursets =
  let f m (asgmt, i) =
    let s =
      match AssignmentMap.find_opt asgmt m with
      | Some s' -> s'
      | None -> IntSet.empty
    in
    let s' =
      if IntSet.mem i s then
        failwith @@ "repeat featureset" ^ string_of_featureset (asgmt, i)
      else IntSet.add i s
    in
    AssignmentMap.add asgmt s' m
  in
...
let readJson () : featursets StringMap.t =
  ...
  let process_entry json =
    ...
    let fs0 =
      json |> member "feature-sets" |> to_list
      |> List.map (compose string_to_featureset to_string)
      |> string_list_to_featursets
    in
    (file, fs0)
  in
