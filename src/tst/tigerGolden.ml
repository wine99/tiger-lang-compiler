...
let runWithStatus cmd =
  let inp, outp, errp = Unix.open_process_full cmd @@ Unix.environment () in
  let i = Stdio.In_channel.input_all inp in
  let e = Stdio.In_channel.input_all errp in
  Stdio.In_channel.close inp ;
  Stdio.Out_channel.close outp ;
  Stdio.In_channel.close errp ;
  let s =
    match e with
    | "" -> i
    | _ -> String.concat "\n" [i; "\n--- StdErr ---"; e]
  in
  (Unix.close_process_full (inp, outp, errp), s)
...
let goldenFrontEnd phase code out file overwrite =
  ...
  | 0, true, false | _, true, _ ->
      ...
      if not (String.equal golden out) then
        DiffWriter.logDiff (Filename.basename file) (phaseName phase)
          golden_file out ;
...
let goldenWithExitCode expectedExitCode phase overwrite file () =
  ...
  | Unix.WEXITED 0 when expectedExitCode = 0 ->
      goldenFrontEnd phase 0 out file
        overwrite (* only check the output on 0-exit *)
  | Unix.WEXITED code when expectedExitCode = 0 ->
      goldenFrontEnd phase code out
  ...
  | Unix.WEXITED 0 -> (
      if phase = HOIST then ()
        ...
        let cmd, runmode =
          match phase with
          | LL r ->
              ( Printf.sprintf "clang %s %s -o %s" runtime_c ll_or_asm_file
                  outfile
              , r )
          | X86 r ->
              ( Printf.sprintf "clang %s %s -o %s" runtime_c ll_or_asm_file
        ...
        | Unix.WEXITED 0 -> (
            if runmode = Batch then
              ...
              | Unix.WEXITED code ->
                  let out_to_compare =
                    if Sys.file_exists (file ^ ".ignore-code") then out
                    else
                      let sep = if out <> "" then "\n" else "" in
                      out ^ sep ^ string_of_int code
                  in
                  if (not (Sys.file_exists golden_file)) || overwrite then
                    Stdio.Out_channel.write_all golden_file
                      ~data:out_to_compare
                  else
                    let golden = Stdio.In_channel.read_all golden_file in
                    if not (String.equal golden out_to_compare) then
                      DiffWriter.logDiff (Filename.basename file)
                        (phaseName phase) golden_file out_to_compare ;
                    Alcotest.(check string)
                      "Matching program output" golden out_to_compare
              | _ ->
                  Alcotest.fail
                    "Exit via an interrupt or a signal is not supported. \
