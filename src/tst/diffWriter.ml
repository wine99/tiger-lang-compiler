...
let logDiff test_name phase_name expected_name got_str =
  ...
  output_string tmp_channel got_str ;
  flush tmp_channel ;
  close_out tmp_channel ;
  let inp =
    Unix.open_process_in
    @@ String.concat " " ["diff -u"; expected_name; tmp_file]
  in
...
let tag name ?(args = []) content =
  let args_str =
    args |> List.map (fun (a, v) -> a ^ "=" ^ v) |> String.concat " "
  in
...
let writeResults out_file =
  ...
  let date =
    Printf.sprintf "%02d-%02d-%04d" tm_mday (tm_mon + 1) (tm_year + 1900)
  in
  let time_str = String.concat " " ["Generated at"; time; "on"; date] in
  let body =
    tag "body"
    @@ tag "font" ~args:[("size", "4")]
    @@ String.concat "<br>\n"
