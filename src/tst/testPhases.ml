...
type runmode =
  | Batch
  | Interactive (* the interactive mode is not yet supported *)

type testphase = LEX | PAR | SEM | HOIST | LL of runmode | X86 of runmode
...
let phase_assignment_agreement p a =
  match (p, a) with
  | LEX, Lex | PAR, Par | SEM, Sem | LL _, LL | X86 _, XAsm -> true
  | _ -> false

let assignment_of_phase p =
  match p with
  | LEX -> Some Lex
  | PAR -> Some Par
  | SEM -> Some Sem
...
let string_of_featureset (a, j) = string_of_assignment a ^ string_of_int j
