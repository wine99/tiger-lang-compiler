...
let as_string max_unfolds e0 =
  ...
  and exp_desc (e, d) =
    ...
    | StringExp s ->
        concat [indent d; "StringExp(\""; String.escaped s; "\")"]
    | CallExp {func; args} ->
        concat
          [ indent d
          ; "CallExp("
    ...
    | RecordExp {fields} ->
        let f ((name, e), d) =
          concat
            [indent d; "("; string_of_symbol name; ",\n"; exp (e, d + 1); ")"]
        in
        concat [indent d; "RecordExp(["; dolist d f fields; "])"]
    | SeqExp l -> concat [indent d; "SeqExp["; dolist d exp l; "]"]
  ...
  and var_desc (e, d) =
    match e with
    | SimpleVar s -> concat [indent d; "SimpleVar("; string_of_symbol s; ")"]
    | FieldVar (v, s) ->
        concat
          [ indent d
          ; "FieldVar(\n"
