...
let pp_str s pp _ = Fmt.string pp s
...
let pp_pos (pos : Lexing.position) =
  let pos_col_relative (pos : Lexing.position) =
    pos.pos_cnum - pos.pos_bol + 1
  in
  Format.asprintf "%s:%d:%d" pos.pos_fname pos.pos_lnum
    (pos_col_relative pos)
...
let ret x =
  let open Fmt in
  hvbox
    (concat [hvbox ~indent:2 (pp_str "return (" ++ cut ++ x); pp_str ");"])

let store v x =
  let open Fmt in
  hvbox (concat [hvbox ~indent:2 (pp_str (v ^ " = ") ++ x ++ pp_str ";")])
...
let arrow x =
  ...
  concat
    [ parens @@ vbox
      @@ concat
           [ vbox ~indent:2
             @@ concat [hbox @@ concat [parens nop; pp_str " => { "]; x]
           ; pp_str "}" ]
    ; pp_str "()" ]
...
let rec exp_to_js (k : 'a Fmt.t -> 'a Fmt.t) pfy (Exp {exp_base; pos; ty}) =
        ...
        (concat
           ...
           ; k
             @@ parens
                  ( pp_str "Math.pow"
                  ++ parens
                       (pp_str left_let ++ pp_str "," ++ pp_str right_let) )
           ] )
        pfy ()
        ...
        (concat
           ...
           ; k
             @@ parens
                  ...
                  ++ pp_str
                       ( match oper with
                       | DivideOp | EqOp | NeqOp | LtOp | LeOp | GtOp | GeOp
                         ->
                           "|0"
                       | _ -> "" ) ) ] )
        pfy ()
  | LetExp {vardecl= VarDec {name; init; _}; body} ->
      let open Fmt in
      vbox (concat [pp_exp (bindsym name) init; pp_exp k body]) pfy ()
  | SeqExp [] -> k' Fmt.nop
         ...
         (concat
            [ vbox ~indent:2
              @@ concat
                   ...
                   ; pp_exp k thn ]
            ; vbox ~indent:2
              @@ concat [hbox @@ concat [pp_str "} else {"]; pp_exp k els]
            ; pp_str "} " ] ) )
        pfy ()
         ...
         (concat
            [ vbox ~indent:2
              @@ concat
                   ...
                   ; pp_exp id thn ]
            ; vbox ~indent:2
              @@ concat [hbox @@ concat [pp_str "} else {"]; pp_exp id els]
            ; pp_str "}" ] ) )
        pfy ()
  ...
  | CallExp {func; lvl_diff; args} ->
      ...
      let arg_lets =
        List.init (List.length args) (fun i ->
            genname ("arg" ^ string_of_int i) )
      in
      let pp_args_lets = pp_str @@ "let " ^ String.concat ", " arg_lets in
      let pp_store_args =
        concat
        @@ List.mapi
             (fun i exp -> pp_exp (store @@ List.nth arg_lets i) exp)
             args
      in
      let pp_args = List.map (fun s -> pp_str s) arg_lets in
      let pp_arg_exps = sl :: pp_args in
      (concat ~sep:semi
         [ ( if List.length args = 0 then nop
        ...
        pfy ()
  | AssignExp {var; exp} -> (pp_exp (setvar var) exp) pfy ()
  | RecordExp {fields} ->
      let open Fmt in
      let field_lets =
        List.init (List.length fields) (fun i ->
            genname ("field" ^ string_of_int i) )
      in
      let pp_field_lets = pp_str @@ "let " ^ String.concat ", " field_lets in
      let pp_store_field i (_, e) =
        pp_exp (store @@ List.nth field_lets i) e
      in
      let pp_store_fields = concat @@ List.mapi pp_store_field fields in
      let pp_fields = List.map (fun s -> pp_str s) field_lets in
      let field_to_js i (s, _) =
        hvbox @@ concat [pp_sym s; pp_str ":"; List.nth pp_fields i]
      in
      let fields = List.mapi field_to_js fields in
      ( concat ~sep:semi
      @@ [ ( if List.length fields = 0 then nop
      ...
      (concat
         ...
         ; k
           @@ concat
                ...
                ; pp_str ")" ] ] )
        pfy ()
  | WhileExp {test= Exp {exp_base= SeqExp test_exps; pos; _}; body}
    when List.length test_exps > 1 ->
      let test_intro, test_res =
        test_exps |> List.rev
        |> fun ls -> (List.rev @@ List.tl ls, List.hd ls)
      in
      let name = genname "cnd" in
      let open Fmt in
      ( vbox
           ...
           [ vbox ~indent:2
             @@ concat
                  [ vbox
                    @@ concat
                         [ pp_exp id
                             (Exp {exp_base= SeqExp test_intro; pos; ty= VOID}
                             )
                         ; pp_str @@ "let " ^ name ^ ";"
       ...
       @@ concat
            [ vbox ~indent:2
              @@ concat
                   ...
                   ; vbox
                     @@ concat ~sep:semi
                          [pp_exp id body; pp_exp (store name) test] ]
            ; pp_str "}" ] )
        pfy ()
  | BreakExp -> pp_str "break;" pfy ()

and pp_exp k e pfy _ = exp_to_js k pfy e

and var_to_js k (pfy : Format.formatter) (Var {var_base; pos; _}) : unit =
  match var_base with
  | AccessVar (0, s) -> k (pp_local s) pfy ()
  | AccessVar (depth, s) -> k (pp_via_sl depth s) pfy ()
...
and pp_var (k : 'a Fmt.t -> 'a Fmt.t) (v : var) pfy _ = var_to_js k pfy v

and bindsym s x =
  let open Fmt in
  hbox @@ concat [pp_local s; sp; pp_str "="; sp; x; pp_str ";"]

and setvar (var : var) x =
  let open Fmt in
  hbox @@ concat [pp_var id var; sp; pp_str "="; sp; x; pp_str ";"]

let fdecl_to_js (ppf : Format.formatter)
    (Fdecl {name; body; args; result= res_ty; _}) =
  let pp_arg_names =
    pp_str _sl :: List.map (fun (Arg {name; _}) -> pp_sym name) args
  in
  let open Fmt in
  Fmt.concat
    [ vbox ~indent:2
...
let to_js file =
  let open Fmt in
  let header =
    Stdio.In_channel.read_all "src/compiler/hoist/html/header.html"
  in
  let footer =
    Stdio.In_channel.read_all "src/compiler/hoist/html/footer.html"
  in
  let title =
    String.concat ""
      ["<script>document.title = 'TigerJS: "; file; "';</script>"]
  in
  vbox ~indent:0
  @@ concat [pp_str header; to_js'; pp_str footer; pp_str title]
