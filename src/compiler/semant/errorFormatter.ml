...
let errorIntRequired ty = "INT required, " ^ string_of_type ty ^ " provided"

let errorVoid = "cannot assign, void is not a value"
...
let errorRecordFieldType field_name t_expec t_given =
  "record field " ^ field_name ^ "has type " ^ t_expec ", but received "
  ^ t_given
...
let errorVariableUndefined id = "undefined variable " ^ string_of_symbol id

let errorVariableUnassignable id =
  "cannot assign to the for-variable " ^ string_of_symbol id

let errorFunctionUndefined id = "undefined function " ^ string_of_symbol id
...
let errorOtherComparison t1 t2 =
  "only strings and ints can be compared with that particular binary \
   operator, " ^ string_of_type t1 ^ ", " ^ string_of_type t2 ^ " provided"

let errorArith = "only ints can be used in arithmetic expressions"

let errorIfThenShouldBeVoid thenTy =
  "then branch should have type void but has type " ^ string_of_type thenTy
...
let errorArrayType ty = "type " ^ string_of_type ty ^ " is not an array type"

let errorArrayInitType ty actTy =
  "type of array initialization is " ^ string_of_type ty ^ " but should be "
  ^ string_of_type actTy

let errorTypeDeclLoop names =
  "illegal circular type declaration: "
  ^ String.concat "::" (List.map string_of_symbol names)

let errorDuplicate name = "duplicate definition " ^ string_of_symbol name
