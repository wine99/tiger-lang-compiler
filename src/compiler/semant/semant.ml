...
let rec transExp ({err; venv; tenv} : context) e =
  let rec trexp (A.Exp {exp_base; pos}) : Tabsyn.exp = (
    ...
    | VarExp var -> (
      ...
      match tvar with TA.Var { ty ; _ } -> TA.Exp { exp_base = VarExp tvar ; pos ; ty } )
    ...
    | A.CallExp {func ; args} -> (
      ...
      | None       -> Err.error err pos (EFmt.errorFunctionUndefined func); err_exp
      | Some tFunc -> (
        match tFunc with
        | FunEntry {formals ; result} -> (
          if (List.length args) != (List.length formals) then
            (Err.error err pos (EFmt.errorFunctionArguments func); err_exp)
          else ((* Check if arguments have correct type *)
            let argTy = List.map (
              fun x ->
                match trexp x with
                | TA.Exp { ty ; _ } -> ty
        ...
        | _ -> Err.error err pos (EFmt.errorUsingVariableAsFunction func); err_exp
      )
    )
  ...
  and trvar (A.Var {var_base; pos}) : TA.var = (
    ...
    | A.FieldVar (v, s) -> let Var { var_base ; pos ; ty } = trvar v in (
      match actual_type err pos ty with
      | RECORD (types, unique) -> raise NotImplemented
      | t -> Err.error err pos (EFmt.errorRecordType t) ; TA.Var { var_base = TA.FieldVar (Var { var_base ; pos ; ty } , s) ; pos ; ty = Ty.ERROR }
    )
...
| NAME (sym, opt_ty_ref) -> (
  ...
  | None   -> Err.error err pos (EFmt.errorTypeDoesNotExist sym) ; Ty.ERROR
