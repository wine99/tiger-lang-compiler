...
let baseVenv =
  ...
  add (S.symbol "substring") (FunEntry { formals = [Ty.STRING; Ty.INT; Ty.INT]; result = Ty.STRING }) |>
  add (S.symbol "concat"   ) (FunEntry { formals = [Ty.STRING; Ty.STRING]     ; result = Ty.STRING }) |>
  add (S.symbol "not"      ) (FunEntry { formals = [Ty.INT]                   ; result = Ty.INT    }) |>
  add (S.symbol "exit"     ) (FunEntry { formals = [Ty.INT]                   ; result = Ty.VOID   })

let baseTenv =
  S.empty |>
  add (S.symbol "int") Ty.INT |>
  add (S.symbol "string") Ty.STRING
